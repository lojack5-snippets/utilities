...
#
#  Wrye Bash copyright (C) 2005-2014 Wrye Bash Team
#
# =============================================================================
...
#--Local
from Optimize import make_constants, bind_all
...
class PathUnion(object):
    ...
    def makedirs(self, allDirs=True):
        """Makes directories for the PathUnion.  By default, it ensures
           the directory exists for all possible locations of the Union.
           If `allDirs` is False, only the first path in the search order
           is created."""
        if allDirs:
            for path in self.dirs:
                path.makedirs()
        else:
            self.dirs[0].makedirs()
    ...
    def join(self, *args):
        """Return Path object from joining directory with names.  How
    ...
    def joinAll(self, *args):
        """Return a list of Path objects from joining directory with names.
           The order of the list is the same order that the files would be
           accessed decided by creation mode."""
        norms = [getNorm(x) for x in args]
        files = (root.join(*norms) for root in self.dirs)
        files = (f for f in files if f.exists)
        if self._mode & PathUnion.MODE_TIMESTAMP:
            return sorted(files, key=lambda x: x.mtime,
                          reverse=not (self._mode & PathUnion.MODE_REVERSE))
        return files
